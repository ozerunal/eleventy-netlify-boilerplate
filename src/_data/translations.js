module.exports =
{
    header: {
        services: {
            'en-GB': 'Services',
            'tr-TR': 'Hizmetler',
            'nl-BE': 'Diensten'
        },
        aboutus: {
            'en-GB': 'About us',
            'tr-TR': 'Hakkımızda',
            'nl-BE': 'Over ons'
        },
        contactus: {
            'en-GB': 'Contact us',
            'tr-TR': 'Bize ulaşın',
            'nl-BE': 'Contacteer ons'
        },
        selectlanguage: {
            'en-GB': 'Select language',
            'tr-TR': 'Dil seçin',
            'nl-BE': 'Kies taal'
        }
    },
    video: {
        title: {
            'en-GB': 'Forwarding',
            'tr-TR': 'Nakliye',
            'nl-BE': 'Expeditie'
        }
    },
    services: {
        shipping: {
            title: {
                'en-GB': 'Shipping & Forwarding',
                'tr-TR': 'Nakliye',
                'nl-BE': 'Expeditie'
            },
            desc: {
                'en-GB': 'Nimbus Forwarding offers a custom made solution for all your forwarding and shipping needs. With out ever growing network, we are certain we can provide the best solution.',
                'tr-TR': 'Nimbus Forwarding, tüm yönlendirme ve nakliye ihtiyaçlarınız için özel olarak hazırlanmış bir çözüm sunar. Sürekli büyüyen ağımızla en iyi çözümü sunabileceğimizden eminiz.',
                'nl-BE': 'Nimbus Forwarding biedt een op maat gemaakte oplossing voor al uw expeditie- en verzendbehoeften. Met ons steeds groter wordende netwerk weten we zeker dat we de beste oplossing kunnen bieden.'
            }
        },
        custom: {
            title: {
                'en-GB': 'Custom Clearance',
                'tr-TR': 'Gümrükleme',
                'nl-BE': 'Douane Inklaring'
            },
            desc: {
                'en-GB': 'As Nimbus Forwarding we pride ourselves to be a reliable and confident partner in dealing with any custom clearance matters. The long years of experience of our staff ensures the best approach.',
                'tr-TR': 'Nimbus Forwarding olarak, herhangi bir gümrükleme konusuyla ilgilenirken güvenilir bir ortak olmaktan gurur duyuyoruz. Personelimizin uzun yıllara dayanan deneyimi, en iyi yaklaşımı sağlar.',
                'nl-BE': 'Als Nimbus Forwarding kunnen we met trots meedelen dat we een betrouwbare en zelfverzekerde partner zijn voor het afhandelen van alle inklaringskwesties. De jarenlange ervaring van onze medewerkers zorgt voor de beste aanpak.'
            }
        },
        transport: {
            title: {
                'en-GB': 'Road transport',
                'tr-TR': 'Karayolu taşımacılığı',
                'nl-BE': 'Baan transport'
            },
            desc: {
                'en-GB': 'Delivering goods on Time and intact. As Nimbus Forwarding we have amassed a vast network of trustworthy partners which can do exactly that.',
                'tr-TR': 'Malları zamanında ve eksiksiz teslim etmek. Nimbus Forwarding olarak, tam olarak bunu yapabilen güvenilir ortaklardan oluşan geniş bir ağ oluşturduk.',
                'nl-BE': 'Goederen op tijd en intact leveren. Als Nimbus Forwarding hebben we een uitgebreid netwerk van betrouwbare partners opgebouwd die precies dat kunnen.'
            }
        }
    },
    cta: {
        title1: {
            'en-GB': 'Contact us.',
            'tr-TR': 'Bize ulaşın.',
            'nl-BE': 'Contacteer ons.'
        },
        title2: {
            'en-GB': 'Ask for a free quotation.',
            'tr-TR': 'Ücretsiz fiyat teklifi isteyin.',
            'nl-BE': 'Vraag vrijblijvend een offerte aan.'
        },
        desc: {
            'en-GB': 'Let us surprise you with our service, prices and many more we have to offer.',
            'tr-TR': 'Hizmetimiz, fiyatlarımız ve sunduğumuz daha birçok şeyle sizi şaşırtalım.',
            'nl-BE': 'Laat u verrassen door onze service, prijzen en nog veel meer.'
        },
        btn: {
            'en-GB': 'Contact us',
            'tr-TR': 'Bize ulaşın',
            'nl-BE': 'Contacteer ons'
        }
    },
    about: {
        title: {
            'en-GB': 'About us',
            'tr-TR': 'Hakkımızda',
            'nl-BE': 'Over ons'
        },
        desc: {
            'en-GB': 'We are a young, dynamic team. Flexibility, service and many more qualities make sure we are always prepared to deliver the utmost standard regarding logistics of any kind.',
            'tr-TR': 'Biz genç, dinamik bir ekibiz. Esneklik, hizmet ve daha birçok nitelik, her türlü lojistik konusunda en yüksek standardı sunmaya her zaman hazır olmamızı sağlar.',
            'nl-BE': 'We zijn een jong, dynamisch team. Flexibiliteit, service en nog veel meer kwaliteiten zorgen ervoor dat we altijd bereid zijn om de hoogste standaard te leveren op het gebied van logistiek van welke aard dan ook.'
        },
        sect1title: {
            'en-GB': 'Dedication',
            'tr-TR': 'Özveri',
            'nl-BE': 'Toewijding'
        },
        sect1desc: {
            'en-GB': 'We strongly believe that through dedication and perseverance, we can overcome any adversity we face and actually add value to whatever it is you may need.',
            'tr-TR': 'Adanmışlık ve sebatla karşılaştığımız her türlü zorluğun üstesinden gelebileceğimize ve ihtiyaç duyabileceğiniz her şeye gerçekten değer katabileceğimize yürekten inanıyoruz.',
            'nl-BE': 'We zijn er sterk van overtuigd dat we door toewijding en doorzettingsvermogen elke tegenslag kunnen overwinnen waarmee we worden geconfronteerd en daadwerkelijk waarde kunnen toevoegen aan wat u ook maar nodig heeft.'
        },
        sect2title: {
            'en-GB': 'Service',
            'tr-TR': 'Hizmet',
            'nl-BE': 'Dienstverlening'
        },
        sect2desc: {
            'en-GB': 'We pride ourselves with our service, being of the utmost caliber. In our line of work, precision and perfection is not a luxury, but a simple necessity.',
            'tr-TR': 'Son derece kalibreli olarak hizmetimizle gurur duyuyoruz. Bizim işimizde hassasiyet ve mükemmellik bir lüks değil, basit bir gerekliliktir.',
            'nl-BE': 'We zijn trots op onze service, die van het hoogste kaliber is. In ons werk is precisie en perfectie geen luxe, maar pure noodzaak.'
        },
        sect3title: {
            'en-GB': 'Flexibility',
            'tr-TR': 'Esneklik',
            'nl-BE': 'Flexibiliteit'
        },
        sect3desc: {
            'en-GB': "Flexibility is a mindset. We don't mind hardship, we don't mind adversity. Everything is a lesson which translates into experience. Using this, we strive to provide quality in every way possible.",
            'tr-TR': 'Esneklik bir zihniyettir. Zorluğa aldırmıyoruz, sıkıntıya aldırmıyoruz. Her şey deneyime dönüşen bir derstir. Bunu kullanarak, mümkün olan her şekilde kaliteyi sağlamaya çalışıyoruz.',
            'nl-BE': 'Flexibiliteit is een mentaliteit. We vinden ontberingen niet erg, we vinden tegenspoed niet erg. Alles is een les die zich vertaalt in ervaring. Hiermee streven we ernaar om op alle mogelijke manieren kwaliteit te leveren.'
        },
    },
    contact: {
        title: {
            'en-GB': 'Contact us.',
            'tr-TR': 'Bize ulaşın.',
            'nl-BE': 'Contacteer ons.'
        },
        desc: {
            'en-GB': 'Send us an e-mail. Give us a call. Send out smoke signals, we will definitely get back to you as soon as possible.',
            'tr-TR': 'Bize bir e-posta gönderin. Bizi arayın. Duman sinyalleri gönderin, kesinlikle en kısa sürede size geri döneceğiz.',
            'nl-BE': 'Stuur ons een e-mail. Bel ons. Stuur rooksignalen uit, we komen zeker zo snel mogelijk bij je terug.'
        },
        calltitle: {
            'en-GB': 'Call us',
            'tr-TR': 'Bizi arayın',
            'nl-BE': 'Bel ons'
        },
        calldesc: {
            'en-GB': 'We are always available',
            'tr-TR': 'Biz her zaman müsaitiz',
            'nl-BE': 'Wij zijn altijd bereikbaar'
        },
        mailtitle: {
            'en-GB': 'Send us a mail',
            'tr-TR': 'Bize mail gönderin',
            'nl-BE': 'Stuur ons een mail'
        },
        maildesc: {
            'en-GB': 'We will reply as fast as possible',
            'tr-TR': 'Mümkün olduğunca çabuk cevap vereceğiz',
            'nl-BE': 'We zullen zo snel mogelijk antwoorden'
        }
    },
    copyright: {
        'en-GB': 'Nimbus Forwarding is part of Nimbus Logistics BV - BE 0787 337 518',
        'tr-TR': "Nimbus Forwarding, Nimbus Logistics BV'nin bir parçasıdır - BE 0787 337 518",
        'nl-BE': 'Nimbus Expeditie is onderdeel van Nimbus Logistics BV - BE 0787 337 518'
    }
}